...
class JobCommand : Command() {
   ...
   koupper job build-worker [queue]       Builds a runnable worker JAR for a queue.
   koupper job run-worker --queue=name    Starts a worker listening to a queue.
   koupper job list [--queue=name]        Lists available jobs (optionally by queue).
   ...
   For more info: https://koupper.com/cli/commands/job
    ...
    override fun execute(vararg args: String): String {
        val validSubcommands = listOf(
            "init", "build-worker", "run-worker", "list", "status", "failed", "retry", "help"
        )
